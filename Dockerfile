...
ADD . /opt/
RUN chmod -R 777 /opt/ 
RUN cd /opt/ && sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 -keyout ftps.key -out ftps.crt -subj "/C=US/ST=CA/L=LG/O=Dis/CN=ftp-hook.com" && cat ftps.crt ftps.key > ftps.pem
RUN mkdir /jail/
